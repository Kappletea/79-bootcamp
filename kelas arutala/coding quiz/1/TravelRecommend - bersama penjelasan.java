...
public class TravelRecommend {
    public static void main(String[] args) {
        ...
        // 2835000
        // 6460000
        long[] totalFeePerDestinasi= menghitungTotalFeePerDestinasi(transportationFee, hotelFee, culinaryFee, rekreasiFee, jumlahOrang,lamaMenginap);
        for(int i =0; i < totalFeePerDestinasi.length; i++){
            System.out.println(totalFeePerDestinasi[i]);
        }
        ...
        // 3665000
        // 40000
        long[] listBudgetTerdekat = menghitungDestinasiTerdekatDenganBudget(totalFeePerDestinasi, budget);
        for(int i =0; i < listBudgetTerdekat.length; i++){
            System.out.println(listBudgetTerdekat[i]);
        }

        System.out.println(" ");
        // cari nilai selisih terkecil lalu cari index nya
        //dari contoh di atas kalau dengan mata bisa terlihat bahwa nilai yang paling terkecil adalah index ke 3
        ...
        System.out.println(" " + totalFeePerDestinasi[indexSelisihTerkecil]);

    }

    public static long[] menghitungTotalFeePerDestinasi(long[] transportationFee, long[] hotelFee, long[] culinaryFee, long[] rekreasiFee, int jumlahOrang, int lamaMenginap){
        long[] result=new long[4];
        for(int i = 0; i<transportationFee.length;i++){
            result[i] = (transportationFee[i] + ((hotelFee[i] + culinaryFee[i])*lamaMenginap) + rekreasiFee[i])* jumlahOrang;
        }
        return result;
    }
